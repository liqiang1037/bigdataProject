...
public class ProcessTest1_KeyedProcessFunction {
    ...
    public static class MyProcess extends KeyedProcessFunction<Tuple, SensorReading, String> {
        ...
        public void processElement(SensorReading value, Context ctx, Collector<String> out) throws Exception {
            out.collect(value.getId().length()+"==="+ctx.timerService().currentProcessingTime());
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            System.out.println(tsTimerState.value() + "=== 时间");

            ctx.getCurrentKey();
            System.out.println("ctx.getCurrentKey()==>"+ctx.getCurrentKey());
            System.out.println("ctx.timeDomain()==>"+ctx.timeDomain());
